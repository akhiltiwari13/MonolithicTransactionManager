...
const rsmq = new RedisSMQ({
  url: envConfig.get('redisUrl'),
  // port: 6379,
  ns: 'rsmq'
})

rsmq.createQueue({ qname: 'nathan-as-receiver' }, (err, resp) => {
  if (resp === 1) logger.debug('nathan-as-receiver queue created')
})
...
class BitsharesAdapter {
  ...
  getAddress = (headers, accountName) =>
    new Promise((resolve, reject) => resolve({ BTS: `hwd${accountName}` }))

  getBalance = (
    headers,
    accountName // headers required for other adapters.
  ) =>
    new Promise(async (resolve, reject) => {
      ...
      return this._getAccountId(`hwd${accountName}`)
        ...
        .then(balance =>
          resolve({
            accountName,
            balance: new BigNumber(balance).div(100000).toString(),
            unit: 'UDOO'
          })
        )
        .catch(reject)
    ...
    new Promise(async (resolve, reject) => {
      ...
      if (query.days && txnType === 'all') {
        for (let day = 1; day <= days; day++) {
          ...
          before = moment(before, 'YYYY-MM-DD')
            .subtract(1, 'days')
            .format('YYYY-MM-DD')
        }
      }
      ...
      if (offset < 0 || limit < 0) {
        return reject(
          new BadRequestError('offset and limit must not be negative')
        )
      }
      const isAccountExists = await this._getUuid(accountName)
      if (!isAccountExists) {
      ...
      const transactionsCount = await TransferRepository.find({
        where: whereCondition
      })

      return resolve({
        transactions,
        totalPages: Math.ceil(transactionsCount.length / limit)
      })
    })
  ...
  createAccount = req =>
    new Promise((resolve, reject) => {
      ...
      if (req.body.name !== _.toLower(req.body.name)) {
        return reject(
          new BadRequestError(
            'Account name must not contain upper case character'
          )
        )
      }
      ...
      return this._registerUserToVault(req)
        ...
        .then(pubKey => {
          ...
          Apis.instance(btsWebSocketUrl, true) // TODO: Replace URL
            ...
            .then(() =>
              Promise.all([FetchChain('getAccount', registrarAccount)])
            )
            .then(res => {
            ...
            .then(async () => {
              let addresses = await new CommonAdapater().getAddress(
                req.headers,
                req.body.name
              )
              const connection = getConnection()
            ...
            .then(user =>
              resolve({ name: req.body.name, uuid: user.vault_uuid })
            )
            .catch(err =>
              reject(new BadRequestError('Error in account creation'))
            )
        })
        .catch(error =>
          reject(
            new BadRequestError('Error while registering account to vault')
          )
        )
    })
  ...
  transfer = req =>
    new Promise(async (resolve, reject) => {
      ...
      const senderUuid = isSenderAccountExists
      ...
      const amount = new BigNumber(req.body.sendAmount)
        .multipliedBy(100000)
        .toNumber()
      const sendAmount = {
        amount,
      ...
      Apis.instance(btsWebSocketUrl, true)
        ...
        .then(() =>
          Promise.all([
            FetchChain(
              'getAccount',
              fromAccount === 'nathan' ? 'nathan' : `hwd${fromAccount}`
            ),
            FetchChain(
              'getAccount',
              toAccount === 'nathan' ? 'nathan' : `hwd${toAccount}`
            ),
        ...
        .then(async res => {
          ...
          const valueUSD = new BigNumber(req.body.sendAmount)
            .multipliedBy(price.USD)
            .toNumber()
        ...
        .then(async txn => {
          if (toAccount === 'nathan') {
            const address = await new EthereumAdapter().getAddress(
              req.headers,
              fromAccount
            )
            await this._insertInQueue(address, amount, senderUuid);
          }
          return txn
        })
        .then(txn =>
          resolve({ TransactionId: txn.txn_id, blockNumber: blockNo })
        )
      ...
      return getRequest(url, {}, headers)
        .then(result => {
          if (
            ...
            result.Message ===
              `There is no data for any of the toSymbols ${currency} .`
          ) {
  ...
  getStatus = (blockchain, txnId) =>
    new Promise(async (resolve, reject) => {
      const connection = getConnection()
      const TransferRepository = connection.getRepository(Transfer)
      const transaction = await TransferRepository.findOne({
        txn_id: txnId,
        coin_id: blockchain
      })
      if (!transaction) {
  ...
  _insertInQueue = (address, amount, senderUuid) =>
    new Promise((resolve, reject) => {
      const queueMessageObj = {
        'eth-wallet': address,
        amount,
        senderUuid
      }
      return rsmq.sendMessage(
        {
          qname: 'nathan-as-receiver',
          message: JSON.stringify(queueMessageObj)
        },
        (err, resp) => {
          if (err) {
            logger.error(err)
          ...
          if (resp) {
            logger.debug(`Message sent. ID: ${resp}`);
            return resolve()
          }
        }
      )
  ...
  _getSignature = (req, trHex, registrarAccount) =>
    new Promise(async (resolve, reject) => {
      ...
      const registrarUuid =
        registrarAccount === 'nathan'
          ? nathanUuid
          : await this._getUuid(registrarAccount)
      const body = {
        coinType: 240,
        path: '',
